...
def create_reviews_file(reviews,output_file):
     pex.save_as(array=reviews, dest_file_name = output_file) # opens the file, writes all the lines,closes it. Check for errors throughout
...
def add_column_to_file(input_file,output_file):
    ...
    for index, row in enumerate(rows[1:]): # this loops through the row starting at index 
